...
13. After the outputs are produced, run BOTH post-analysis steps of the
    ...
    a. `save_feature_catalog` (catalogPath=`outputs/background-analysis-output/feature-catalog.json`) — persist the catalog to PostgreSQL.
    b. `generate_decision_list` (same catalogPath) — write `feature-decision-list.xlsx`
       for business to-be adoption discussion. CLI: `node mcp-servers/feature-catalog-store/src/gen-decision-list.js outputs/background-analysis-output/feature-catalog.json`
