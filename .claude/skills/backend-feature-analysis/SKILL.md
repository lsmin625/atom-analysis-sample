...
1. **DB 저장** — `save_feature_catalog` (catalogPath=`outputs/backend-analysis-output/feature-catalog.json`)
...
2. **기능 결정 목록 생성** — `generate_decision_list` (catalogPath 동일)
   ...
   - CLI 대체: `node mcp-servers/feature-catalog-store/src/gen-decision-list.js outputs/backend-analysis-output/feature-catalog.json`
