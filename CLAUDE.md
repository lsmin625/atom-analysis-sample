...
- 분석 산출물의 모든 서술은 한국어로 작성한다(아래 "Output language" 참조).

## Output language

애플리케이션 종류·분석 유형과 무관하게(향후 추가되는 분석 유형 포함)
**모든 분석 산출물의 서술형 텍스트는 한국어로 작성한다.** 대상 소스가
영어로 작성되어 있어도 분석 설명은 한국어로 쓴다.

- 한국어 대상: 기능 설명(`description`), 요약, 근거 서술, 커버리지·미해결
  항목 설명, 결정 목록의 업무 관점 서술 등 사람이 읽는 모든 문장.
- 원문 유지 대상: 식별자·심볼(파일 경로, 클래스·메서드·함수·변수명,
  API 경로, HTTP 메서드, 라우트, DB 컬럼), 열거형 값(예: `HIGH`/`LOW`),
  스키마의 키 이름·ID 형식(`FEAT-...`). 이들은 번역하지 않는다.
- 기술 용어는 한국어 문장 안에 원어를 병기해도 되나(예: "컨트롤러가
  요청을 검증(validation)한다"), 문장 자체는 한국어여야 한다.
- 이 규칙은 각 Skill·서브에이전트·서브에이전트 산출물 전반에 적용된다.
