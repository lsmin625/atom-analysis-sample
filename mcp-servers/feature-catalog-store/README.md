...
```
save_feature_catalog(catalogPath="outputs/oas-doc-ui/feature-catalog.json")
```
...
```
generate_decision_list(catalogPath="outputs/oas-doc-ui/feature-catalog.json")
```

MCP 도구를 호출할 수 없는 환경에서는 동일 기능의 CLI 로 대체한다:

```bash
node src/gen-decision-list.js outputs/oas-doc-ui/feature-catalog.json
# 또는: npm run gen-decision-list -- outputs/oas-doc-ui/feature-catalog.json
```
